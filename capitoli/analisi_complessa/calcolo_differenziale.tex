...
Il calcolo differenziale è alla base dell'analisi in ogni campo, quindi è solamente giusto iniziare dalla nozione di \textit{funzioni differenziabili in senso complesso}. Dato che la struttura sui complessi è `più forte' di quella di $\R^2$, anche se la topologia dei complessi viene indotta da quella di $\R^2$, si avranno teoremi più forti e generali partendo da ipotesi più deboli rispetto a quelli dell'analisi reale. Per esempio una funzione $C^1(\R)$ non è detto che sia $C^2(\R)$, mentre nei complessi se è differenziabile è anche analitica. 
