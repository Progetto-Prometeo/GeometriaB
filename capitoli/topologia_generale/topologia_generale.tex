...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\item Siano dati $\mathcal{Y} \subset 2^X$ e la famiglia $\tau_\cap$ con la seguente restrizione 
		\begin{equation*}
		\mathcal{Y} \subset \tau_i \ \ \forall i 
		\end{equation*}
		allora  $\tau_\cap$ è la topologia meno fine su $X$ tale da contenere $\mathcal{Y}$.
	\end{enumerate}
\end{proposition}
\begin{proof} \
	\begin{enumerate}
...
\begin{proof}
	...
	\end{equation*}
	Per l'ultima inclusione: sia $x$ appartenente all'unione degli $A_i$, allora appartiene ad un $A_i$ per $i \in \mathcal{I}$; poiché ho una base numerabile della topologia esiste un $B_i$ elemento della base tale che $x \in B_i \subset A_i$. In particolare ottengo che $B_i$ appartiene alla famiglia degli $A_j$ perché rispetta le condizioni con cui l'ho costruita, $B_i=B_{j_i}$ per un certo $j_i \in \mathcal{J}$. \\ Da questo
	\begin{equation*}
	B_i=B_{j_i} \subset A_{j_i} \subset \bigcup_{j \in \mathcal{J}}  A_j
	\end{equation*}
\end{proof}
...
\begin{proof}
	...
	\begin{align*}
	A \cap f(D) & \supset f(D \cap f^{-1}(A)) \\
	& \neq \varnothing
	\end{align*}
		%todo: verificare pls perché sono stanco (s)
		%todo: inoltre mi sono permesso di metterlo in align, se ti fa schifo togli
\end{proof}
...
\begin{proof} \
	\begin{enumerate}
		...
		\begin{equation*}
		f^{-1}\left(g^{-1}(A)\right)=\left(g \circ f\right)(A)
		\end{equation*}
...
\end{proof}
Nella prossima pagina ci aspetta una fantastica ricetta.
...
\paragraph{Cosa caspita ho appena letto?}\ \\
\begin{lemma}
	\label{lem_qdom}
