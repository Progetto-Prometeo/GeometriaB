...
\begin{remark}
	È ovvio che l'ultima condizione si può riscrivere come segue: dati $A_1, A_2 \in \tau$ allora $A_1 \cap A_2 \in \tau$. \\ Infatti per induzione si può ottenere di nuovo il terzo assioma. 
\end{remark}
...
\begin{enumerate}
	...
	\marginpar{\textcolor{TopGener}{\textbf{Topologia banale, discreta, cofinita}}}
	\item La topologia più povera in assoluto è la \textcolor{TopGener}{topologia banale}, ovvero $\tau = \left\{\varnothing, X\right\}$. \\ Vediamo che non è metrizzabile se non nel caso in cui $X = \left\{x\right\}$ ovvero sia un \textit{singoletto}. Infatti sia $d$ una distanza su $X$ e sia $X = \left\{p,q\right\}$ allora per ogni $\rho > 0 \; . \; q \in B_\rho(p)$, quindi $d(p,q) = 0$. Ma poiché $d$ è una distanza ottengo che $p=q$, contraddicendo l'ipotesi $p \neq q$. 
	\item Dall'altra parte della barricata c'è la \textcolor{TopGener}{topologia discreta}, ovvero $\tau = 2^X$, che è la più ricca topologia possibile: infatti se $\tau$ è una topologia ho che $\tau \subseteq 2^X$. \\ Si tratta ovviamente di una topologia (la verifica è immediata) e vale anche l'assioma della stabilità per intersezioni di famiglie arbitrarie. \\ Inoltre è sempre metrizzabile per ogni $X \neq \varnothing$: infatti basta prendere la distanza $d(x,y) = 1$ se $x \neq y$. \footnote{si può anche prendere un valore arbitrario, come $69.42^{0.314159}$.}
	\item Definiamo la \textcolor{TopGener}{topologia cofinita} come segue:
	\begin{equation*}
...
Poiché le topologie possono essere molto complesse sarebbe utile avere un sottoinsieme che la generi per unioni, un po' come i generatori dei gruppi o le basi per gli spazi vettoriali. \\ Ed è esattamente qui che intervengono le basi topologiche.
...
\begin{remark}
	Se nella proposizione precedente si sostituisce quel \textit{qualunque A} con tutto l'insieme $X$, si ha che $\mathcal{S}$ forma un ricoprimento di $X$.
\end{remark}
...
\begin{definition}
	Una famiglia $\mathcal{V}(x)$ di intorni di $x$ in $(X,\tau)$, cioè $\mathcal{V}(x) \subset \mathcal{N}_\tau(x)$; si dice \textbf{sistema fondamentale di intorni} di $x$ in $(X,\tau)$ se $\forall U \in \mathcal{N}_\tau(x)\ .\ \exists V \in \mathcal{V}(x)$ tale che $V \subset U$.
\end{definition}
...
\begin{remark}
	Sia $(X, \tau)$ uno spazio topologico, $\forall x \in X$ esiste $\mathcal{V}(x)$ sistema fondamentale di intorni di $x$ in $\tau$.
	\begin{enumerate}
...
\end{lemma}
\begin{proof}
	Infatti sia $\mathcal{B}$ una base numerabile di $\tau$, allora mi basta prendere come sistema fondamentale di intorni tutti i $\left\{B_i\right\} \in \mathcal{B}$ tali che $x \in B_i$. Per cui $\left\{B_i\right\}$ genera un sistema fondamentale di intorni numerabile per ogni $x \in X$.
\end{proof}
...
\section{Sottoinsiemi e chiusura di uno spazio topologico}
...
\chapter{Applicazioni e sottospazi in topologia}
...
\begin{theorem}
	...
	\marginpar{\textbf{\textcolor{TopGener}{La proprietà universale degli spazi quozienti:}} perché è interessante questa proprietà universale? Innanzitutto perché è universale (vale su tutte le topologie quozienti), inoltre perché permette di by-passare la topologia quoziente per verificare che una mappa è continua, senza dover lavorare sulla topologia quoziente stessa.}
